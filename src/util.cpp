...
const char * const BITCOIN_CONF_FILENAME = "ltucoin.conf";
const char * const BITCOIN_PID_FILENAME = "ltucoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "ltucoin" is a composite category enabling all Ltucoin-related debug output
            if(ptrCategory->count(string("ltucoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\LtucoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\LtucoinCore
    // Mac: ~/Library/Application Support/LtucoinCore
    // Unix: ~/.ltucoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "LtucoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/LtucoinCore";
#else
    // Unix
    return pathRet / ".ltucoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ltucoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override ltucoin.conf
