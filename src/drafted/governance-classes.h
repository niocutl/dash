 ...
 *
 *   // network
 *   CLtucoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.ltucoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CLtucoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CLtucoinNetwork(UniValue objIn)
    {
...
// // can be under: LtucoinNetwork
// //   -- signature requirements : Key1(User)
// class CLtucoinNetworkVariable : public CGovernanceObject
// {
// private:
